  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        dropoff_postal_code: normalizePostal(dropoff_postal_code),
        facility_name: facility_name || null,
        facility_unit: facility_unit || null,
        pickup_instructions: pickup_instructions || null,
        appointment_time: appointment_time || null,
        is_round_trip: typeof is_round_trip === "boolean" ? is_round_trip : null,
    ...
    if (!effectivePI) {
      try {
        ...
        if (!existingConfirmation) {
          const serviceLabel = serviceTypeArr.join(", ") || "Home Care";
          const cityOrPostal = (patient_address || client_address || "").split(",").slice(-2, -1)[0]?.trim()
            || normalizedPatientPostal || normalizedClientPostal || "Ontario";
          const firstName = client_first_name || client_name?.split(" ")[0] || "there";
...
.next li::before{content:"\\2713 ";font-weight:bold}
...
<div class="details">
<div class="row"><span class="label">Order Reference</span><span class="val">${data.booking_code}</span></div>
<div class="row"><span class="label">Service Type</span><span class="val">${serviceLabel}</span></div>
<div class="row"><span class="label">Date</span><span class="val">${scheduled_date}</span></div>
<div class="row"><span class="label">Time</span><span class="val">${start_time} – ${end_time}</span></div>
<div class="row"><span class="label">Duration</span><span class="val">${computedHours}h</span></div>
...
<div class="ftr"><p>PSW Direct — Private Home Care, Ontario<br/>(249) 288-4787 · pswdirect.com</p></div>
          ...
          console.log("⏭️ Order confirmation already sent for", data.booking_code);
        }
      } catch (confirmErr) {
