...
BEGIN
  ...
  IF NOT v_did_update THEN
    -- Race: re-read to see if it's now already checked in
    SELECT * INTO v_row FROM public.bookings WHERE id = p_booking_id;
    IF v_row.checked_in_at IS NOT NULL THEN
      RETURN jsonb_build_object(
        'success', true,
...
GRANT EXECUTE ON FUNCTION public.check_in_to_shift(uuid, double precision, double precision, text, boolean, double precision, double precision) TO authenticated, service_role;
