-- ============================================================
-- Phase 8 remediation: single approved provider rate, fail-closed
-- ============================================================

-- 1) Approved-rate resolvers (fail closed: NULL when no approved rate)
CREATE OR REPLACE FUNCTION public.resolve_provider_pay_rate(
...
AS $$
  SELECT public.provider_rate_cents(p_province, p_provider_type)::numeric / 100.0;
$$;

-- Legacy signature kept for compatibility; service type NO LONGER affects pay.
...
SET search_path TO 'public'
AS $$
  SELECT public.resolve_provider_pay_rate('ON', 'psw');
$$;

REVOKE ALL ON FUNCTION public.resolve_provider_pay_rate(text, text) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.resolve_psw_pay_rate(text[]) FROM PUBLIC, anon, authenticated;

-- 2) Remove the obsolete guessing pay calculator ($21 default)
DROP FUNCTION IF EXISTS public.psw_pay_cents(numeric, numeric);

-- 3) Booking rate lock: always the approved rate, never a guess
...
BEGIN
  ...
  v_approved := public.resolve_provider_pay_rate(
    COALESCE(NULLIF(NEW.service_province, ''), 'ON'), 'psw'
  );

  IF TG_OP = 'INSERT' THEN
    -- Client/admin/edge supplied values are ignored entirely.
  ...
  IF v_trusted = 'on' THEN
    -- Authorised correction workflow: re-apply the approved rate only.
    NEW.psw_pay_rate := v_approved;
  ELSE
...
-- 4) No arbitrary admin rate overrides
CREATE OR REPLACE FUNCTION public.admin_set_psw_pay_rate(
  p_booking_id uuid, p_new_rate numeric, p_reason text
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  v_approved := public.resolve_provider_pay_rate(v_province, 'psw');
  ...
  VALUES ('psw_pay_rate_resync', v_actor, p_booking_id, v_code, v_approved, btrim(p_reason),
    jsonb_build_object('previous_rate', v_old, 'new_rate', v_approved,
                       'admin_user_id', auth.uid(), 'changed_at', now()));

  RETURN jsonb_build_object('ok', true, 'previous_rate', v_old, 'new_rate', v_approved);
END;
$$;

-- 5) Payable-hours correction: reason required, consistent cents, audit preserved
...
BEGIN
  ...
  IF v_entry.status IN ('paid', 'paid_manually') THEN
  ...
  -- Preserve the previous basis as superseded audit evidence.
...
-- 6) Dollars and cents can never diverge; paid entries are immutable
...
AS $$
BEGIN
  IF TG_OP = 'UPDATE' AND OLD.status IN ('paid', 'paid_manually') THEN
...
DROP TRIGGER IF EXISTS trg_enforce_payroll_amount_consistency ON public.payroll_entries;
CREATE TRIGGER trg_enforce_payroll_amount_consistency
BEFORE INSERT OR UPDATE ON public.payroll_entries
FOR EACH ROW EXECUTE FUNCTION public.enforce_payroll_amount_consistency();

-- 7) Read access: rate table is admin-only; fix caregiver snapshot identity
...
CREATE POLICY "admins and owning provider read earning snapshots"
ON public.provider_earning_snapshots
...
-- 8) Office-only live self-test of the rules above
...
DECLARE
  ...
  v_entry uuid;
BEGIN
  -- T1: Doctor Escort resolves to the approved rate, never 27
  ...
  -- T4: Nurses and every Alberta type have no rate (pending verification)
  ...
  -- T5: obsolete guessing overload removed
  ...
  -- T6: no rate / no minutes produces no amount
  ...
  -- T8: missing approved rate fails closed through the booking trigger
  ...
  -- T9: booking trigger overwrites a supplied 27 with the approved rate
  v_pass := false; v_detail := '';
  ...
  -- T10: dollars and cents cannot diverge
  v_pass := false; v_detail := '';
  ...
  IF v_entry IS NOT NULL THEN
    BEGIN
      ...
      SELECT gross_cents INTO v_int FROM public.payroll_entries WHERE id = v_entry;
      SELECT round(total_owed*100)::int INTO v_num FROM public.payroll_entries WHERE id = v_entry;
  ...
  -- T11: paid entries are immutable
  v_pass := false; v_detail := '';
  IF v_entry IS NOT NULL THEN
    BEGIN
      UPDATE public.payroll_entries SET status = 'paid' WHERE id = v_entry;
      BEGIN
  ...
  -- T12: payable-hours correction requires a reason
  v_pass := false; v_detail := '';
  BEGIN
  ...
  -- T13: arbitrary booking rate overrides are rejected
  v_pass := false; v_detail := '';
  BEGIN
  ...
  -- T14: rate table not readable by ordinary authenticated users
  ...
  -- T15: Alberta remains disabled and the checklist stays incomplete
