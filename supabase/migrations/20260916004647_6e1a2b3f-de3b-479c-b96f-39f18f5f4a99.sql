-- Lock down the new trigger helper
REVOKE ALL ON FUNCTION public.block_earning_snapshot_mutation() FROM PUBLIC, anon, authenticated;

-- ============================================================
-- Authoritative pay maths: requested minutes x approved rate.
-- No fallback rate. NULL means "pending verification".
-- ============================================================
CREATE OR REPLACE FUNCTION public.psw_pay_cents(p_minutes integer, p_rate numeric)
RETURNS integer
LANGUAGE sql
IMMUTABLE
...
AS $$
  SELECT CASE
    ...
    ELSE (round(p_minutes * p_rate * 100 / 60))::int
  END;
$$;
...
AS $$
  ...
  prov AS (
    SELECT lower(coalesce(NULLIF(p.provider_type,''), 'psw')) AS provider_type,
           coalesce(NULLIF(p.province,''), 'ON') AS province
    FROM public.psw_profiles p, me
    WHERE p.id = me.psw_id
    LIMIT 1
  ),
  visible AS (
    SELECT b.id
  ...
  dur AS (
    SELECT b.id,
           ...
           (
             public.provider_rate_cents(
               coalesce((SELECT province FROM prov), coalesce(NULLIF(b.service_province,''),'ON')),
               coalesce((SELECT provider_type FROM prov), 'psw')
             )::numeric / 100.0
           ) AS rate
...
REVOKE ALL ON FUNCTION public.psw_pay_estimates(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.psw_pay_estimates(uuid) TO authenticated, service_role;

-- ============================================================
-- Payroll entry creation under the Phase 8 rule
-- ============================================================
...
BEGIN
  ...
  -- CLIENT-REQUESTED duration only (never clocked / GPS / care-sheet time).
  ...
  IF v_effective_in IS NOT NULL AND v_effective_out IS NOT NULL AND v_effective_out > v_effective_in THEN
    ...
    -- Early sign-out never reduces pay; it flags the visit for office review.
  ...
  -- Rate comes ONLY from the approved provincial rate table.
  ...
  -- Office-approved duration change (never an attendance-derived increase).
  ...
  ON CONFLICT (shift_id) DO UPDATE SET
    ...
    compensation_snapshot = EXCLUDED.compensation_snapshot,
  ...
  -- Immutable snapshot: supersede any earlier one for this booking.
  ...
  IF NOT EXISTS (
    ...
    INSERT INTO public.provider_earning_snapshots (
      ...
      'phase8-v1',
      COALESCE(v_review_reason, 'completed_visit'), 'system'
    );
  END IF;
END;
$$;
