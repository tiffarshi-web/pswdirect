...
BEGIN
  ...
  -- 7-day eligibility from first completed shift (was 14)
  ...
  -- 7-day holdback on individual shift earnings (was 14)
  ...
  IF v_count = 0 OR v_total <= 0 THEN
    RETURN jsonb_build_object('error', 'no_eligible_earnings', 'message', 'No earnings are eligible yet (shifts must be completed 7+ days ago).');
  END IF;
