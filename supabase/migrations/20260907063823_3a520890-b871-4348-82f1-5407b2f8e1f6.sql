...
BEGIN
  ...
  -- Only clients are restricted here. Caregivers and staff flows are governed
  -- by their own policies and SECURITY DEFINER RPCs.
  v_is_client := COALESCE(OLD.user_id = auth.uid(), false)
