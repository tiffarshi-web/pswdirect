...
-- Add a helper to redact PII (phone/email/URL) from free-text before showing
-- it to approved PSWs during pre-acceptance job browsing.
CREATE OR REPLACE FUNCTION public.redact_pii_text(t text)
RETURNS text
LANGUAGE sql
IMMUTABLE
...
AS $$
  SELECT CASE
    WHEN t IS NULL OR length(t) = 0 THEN t
    ELSE
      regexp_replace(
        regexp_replace(
          regexp_replace(
            ...
            '(https?://|www\.)[^\s]+', '[link hidden]', 'g'),
          '(\+?1[\s.\-]?)?\(?\d{3}\)?[\s.\-]?\d{3}[\s.\-]?\d{4}', '[contact hidden]', 'g'),
        '\y\d{7,}\y', '[contact hidden]', 'g')
  END;
$$;

GRANT EXECUTE ON FUNCTION public.redact_pii_text(text) TO authenticated, service_role;

-- Recreate the PSW-safe view: expose redacted special_notes to approved PSWs
-- BEFORE acceptance (contact info stripped); expose full text only to the
-- assigned PSW or admins AFTER acceptance. Exact patient/pickup/dropoff
-- addresses and client PII remain hidden until assignment.
...
SELECT b.id,
  b.booking_code, b.status, b.payment_status, b.stripe_payment_intent_id,
  b.is_asap, b.psw_assigned, b.psw_first_name, b.psw_photo_url,
  b.psw_vehicle_photo_url, b.psw_license_plate,
  b.preferred_languages, b.preferred_gender, b.is_transport_booking,
  b.service_type, b.scheduled_date, b.start_time, b.end_time, b.hours,
  b.care_conditions, b.care_conditions_other, b.is_recurring,
  b.parent_schedule_id, b.flagged_for_overtime, b.overtime_minutes,
  b.suggested_billable_hours, b.final_billable_hours,
  b.psw_cancel_reason, b.psw_cancelled_at,
  b.created_at, b.updated_at, b.cancelled_at, b.cancellation_reason,
  b.patient_postal_code, b.client_postal_code, b.pickup_postal_code,
  b.service_latitude, b.service_longitude, b.geocode_source,
  ...
  CASE
    ...
    WHEN ctx.approved AND b.psw_assigned IS NULL THEN public.redact_pii_text(b.special_notes)
    ELSE NULL::text
...
WHERE ctx.is_admin
   ...
   OR (ctx.approved
       ...
       AND COALESCE(b.status,'') <> ALL (ARRAY['cancelled','archived']));
