    ...
    <div className="space-y-6">
      ...
      <Card className="shadow-card">
        <CardHeader>
          ...
          <CardDescription>
            All tasks with their allotted time, base cost, and tax status. Used to calculate suggested booking duration.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-center">HST</TableHead>
                <TableHead className="text-right">Total w/ Tax</TableHead>
                <TableHead className="text-center">Category</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {taskList.map((task) => {
                const hstAmount = task.applyHST ? task.baseCost * 0.13 : 0;
                const totalWithTax = task.baseCost + hstAmount;
                return (
                  <TableRow key={task.id}>
                    ...
                    <TableCell className="text-right">
                      <span className="font-semibold text-green-600">${task.baseCost.toFixed(2)}</span>
                    </TableCell>
                    <TableCell className="text-center">
                      <Badge 
                        variant={task.applyHST ? "default" : "secondary"}
                        className={`text-xs ${task.applyHST ? 'bg-blue-600 hover:bg-blue-700' : 'bg-green-100 text-green-700 hover:bg-green-200'}`}
                      >
                        {task.applyHST ? "+13%" : "Exempt"}
                      </Badge>
                    </TableCell>
                    <TableCell className="text-right">
                      <span className={`font-semibold ${task.applyHST ? 'text-blue-600' : 'text-green-600'}`}>
          ...
          {/* Task Cost Summary Box */}
          {(() => {
            const subtotal = taskList.reduce((sum, t) => sum + t.baseCost, 0);
            const taxableAmount = taskList.filter(t => t.applyHST).reduce((sum, t) => sum + t.baseCost, 0);
            const hstTotal = taxableAmount * 0.13;
            const grandTotal = subtotal + hstTotal;
            const exemptCount = taskList.filter(t => !t.applyHST).length;
            const taxableCount = taskList.filter(t => t.applyHST).length;
            
            return (
              <Card className="border-2 border-amber-300 bg-amber-50/50 dark:bg-amber-950/30">
                ...
                <CardContent className="space-y-3">
                  <div className="flex justify-between items-center py-1">
                    <span className="text-muted-foreground">Subtotal (all tasks):</span>
                    <span className="font-semibold">${subtotal.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between items-center py-1">
                    <span className="text-muted-foreground">Taxable Items:</span>
                    <span className="font-medium text-blue-600">${taxableAmount.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between items-center py-1">
                    <span className="text-muted-foreground">HST (13% of taxable):</span>
                    <span className="font-medium text-blue-600">${hstTotal.toFixed(2)}</span>
                  </div>
                  <Separator className="my-2" />
                  <div className="flex justify-between items-center py-2 bg-amber-100 dark:bg-amber-900/40 -mx-4 px-4 rounded">
                    <span className="font-bold text-lg">GRAND TOTAL:</span>
                    <span className="font-bold text-xl text-amber-700 dark:text-amber-400">${grandTotal.toFixed(2)}</span>
                  </div>
                  <Separator className="my-2" />
                  <div className="flex justify-between items-center text-sm">
                    <div className="flex items-center gap-2">
                      <Badge variant="secondary" className="bg-green-100 text-green-700">Exempt</Badge>
                      <span className="text-muted-foreground">{exemptCount} tasks</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Badge className="bg-blue-600">+13%</Badge>
                      <span className="text-muted-foreground">{taxableCount} tasks</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
