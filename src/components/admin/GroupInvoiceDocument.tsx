// Grouped multi-day Home Care invoice — client-side renderer.
// Mirrors supabase/functions/_shared/groupInvoiceHtml.ts so the admin preview
// and the emailed document are identical. Home Care is never taxable and never
// carries parking; both lines are rendered explicitly.
...
export const generateGroupInvoiceHtml = (d: GroupInvoiceData): string => {
  const rows = d.visits
    .map(
      (v) => `<tr>
  ...
  <div class="body">
    ...
    <div class="totals">
      ...
      <div><span class="muted">Payment date</span><span>${esc(
        d.paymentDate
          ? new Date(d.paymentDate).toLocaleDateString("en-CA", { year: "numeric", month: "long", day: "numeric" })
          : "—",
      )}</span></div>
...
/** Build renderer input from a stored grouped invoice row. */
export const buildGroupInvoiceData = (invoice: any): GroupInvoiceData => {
  ...
  return {
    ...
    subtotal: Number(snap.subtotal ?? invoice?.subtotal ?? 0),
    total: Number(invoice?.total ?? 0),
    amountPaid: Number(snap.amountPaid ?? invoice?.total ?? 0),
    paymentDate: snap.paymentDate || invoice?.paid_at,
    paymentIntentId: snap.stripePaymentIntentId || invoice?.stripe_payment_intent_id,
    currency: invoice?.currency || "CAD",
  };
};

const openHtml = (html: string, print = false) => {
...
export const viewGroupInvoice = (invoice: any) =>
  openHtml(invoice?.html_snapshot || generateGroupInvoiceHtml(buildGroupInvoiceData(invoice)));

export const downloadGroupInvoicePdf = (invoice: any) =>
  openHtml(invoice?.html_snapshot || generateGroupInvoiceHtml(buildGroupInvoiceData(invoice)), true);
