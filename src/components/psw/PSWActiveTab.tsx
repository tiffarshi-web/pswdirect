...
import { getActiveShifts, type ShiftRecord } from "@/lib/shiftStore";
...
export const PSWActiveTab = ({ onSelectShift }: PSWActiveTabProps) => {
    ...
    const timer = setInterval(() => {
      ...
      activeShifts.forEach(shift => {
        if (shift.checkedInAt) {
          ...
          const now = Date.now();
          newElapsed[shift.id] = Math.floor((now - checkInTime) / 1000);
        }
      });
  ...
  const loadShifts = () => {
    ...
    console.log("PSWActiveTab - Loading active shifts for PSW ID:", pswId);
    const shifts = getActiveShifts(pswId);
    console.log("PSWActiveTab - Found active shifts:", shifts);
