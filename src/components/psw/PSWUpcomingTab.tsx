...
export const PSWUpcomingTab = ({ onSelectShift }: PSWUpcomingTabProps) => {
  ...
  const isLateCancellation = (shift: ShiftRecord): boolean => {
    ...
    return (shiftStart.getTime() - Date.now()) / (1000 * 60 * 60) < 24;
  };
  ...
  if (upcomingShifts.length === 0) {
    return (
      <div className="space-y-4">
        <div><h2 className="text-xl font-semibold text-foreground">My Schedule</h2><p className="text-sm text-muted-foreground mt-1">Your upcoming claimed shifts</p></div>
        <div className="text-center py-12">
          <div className="w-16 h-16 rounded-full bg-muted flex items-center justify-center mx-auto mb-4"><Calendar className="w-8 h-8 text-muted-foreground" /></div>
          <h3 className="text-lg font-medium text-foreground mb-2">No Upcoming Shifts</h3>
          <p className="text-muted-foreground mb-4">Claim available shifts to add them to your schedule</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div><h2 className="text-xl font-semibold text-foreground">My Schedule</h2><p className="text-sm text-muted-foreground mt-1">{upcomingShifts.length} upcoming shift{upcomingShifts.length !== 1 ? "s" : ""}</p></div>
              ...
              <CardContent className="p-4">
                ...
                <div className="space-y-2 text-sm mb-3">
                  ...
                  <div className="flex items-center gap-2 text-muted-foreground">
                    <MapPin className="w-4 h-4" /><span className="flex-1">{shift.patientAddress}</span>
                    <Button variant="ghost" size="sm" className="h-6 px-2" onClick={(e) => launchNavigation(shift.patientAddress, e)}>
                  ...
                  <div className="flex items-center gap-2 text-muted-foreground">
                    ...
                    <Button variant="outline" size="sm" className="h-6 px-2 text-primary border-primary/30" onClick={(e) => callOffice(e)}>
                      <Phone className="w-3 h-3 mr-1" />Call Office
                    </Button>
                  </div>
                </div>
