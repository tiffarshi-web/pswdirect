...
/**
 * Create a mock hospital discharge document for testing
 * Returns a base64-encoded HTML document simulating discharge papers
 */
export const createMockDischargeDocument = (): { base64: string; fileName: string } => {
      ...
      <body style="font-family: Arial, sans-serif; padding: 40px; max-width: 600px; margin: 0 auto;">
        <div style="border: 2px solid #0077B5; padding: 20px; border-radius: 8px;">
          ...
          <ul>
            ...
            <li>Return to ER if experiencing severe pain, difficulty breathing, or fever over 38.5°C</li>
          </ul>
          ...
          <div style="background: #fff3cd; border-left: 4px solid #ffc107; padding: 16px; margin: 20px 0;">
            <p style="margin: 0; color: #856404;">
              <strong>⚠️ THIS IS A MOCK DOCUMENT FOR TESTING PURPOSES ONLY</strong>
            </p>
            <p style="margin: 8px 0 0 0; font-size: 12px; color: #856404;">
              Generated by PSW Direct E2E Testing System
            </p>
          </div>
  ...
  const base64Content = btoa(mockHtmlContent);
  const dataUrl = `data:text/html;base64,${base64Content}`;
  
  return {
    base64: dataUrl,
    fileName: `mock-discharge-${Date.now()}.html`
  };
};
